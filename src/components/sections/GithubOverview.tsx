...
import { Github, Star, GitFork, Code2, Layers } from 'lucide-react';
// Perbaikan: Menggunakan Named Export { GitHubCalendar }
        ...
        if (Array.isArray(reposData)) {
          ...
          const langStats: LanguageStat[] = Object.keys(langMap)
            .map((lang) => ({
              name: lang,
              count: langMap[lang],
              percentage: totalReposWithLang > 0 ? Math.round((langMap[lang] / totalReposWithLang) * 100) : 0,
          ...
          console.warn('API GitHub mengembalikan objek, bukan array (kemungkinan Rate Limit).');
          setLanguages([]);
        }
    ...
    <section className="py-24 bg-background transition-colors duration-500">
      <div className="container mx-auto px-6 max-w-7xl">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 p-8 rounded-[2.5rem] bg-card border border-border backdrop-blur-md shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                ...
                <div className="flex gap-10">
                  <div className="flex flex-col">
                    <span className="text-3xl font-black text-emerald-500 tabular-nums">{loading ? '-' : profile?.public_repos || 0}</span>
                    <span className="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Repos</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-3xl font-black text-emerald-500 tabular-nums">{loading ? '-' : profile?.followers || 0}</span>
              ...
              <div className="w-full overflow-x-auto no-scrollbar py-2">
                <div className="min-w-175 flex justify-center p-6 bg-background/50 rounded-3xl border border-border">
                  {mounted ? (
                    ...
                    <div className="h-37.5 flex items-center justify-center text-muted-foreground font-mono text-xs uppercase tracking-widest">Booting System...</div>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-1 p-8 rounded-[2.5rem] bg-card border border-border backdrop-blur-md shadow-sm flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <div className="mb-10 scale-110">
              {!loading && languages.length > 0 ? (
                ...
                <div className="w-40 h-40 rounded-full border-4 border-dashed border-border flex items-center justify-center text-[10px] font-bold text-slate-500 uppercase tracking-widest">{loading ? 'Analyzing...' : 'No Data'}</div>
              )}
            </div>
          ...
          <div className="lg:col-span-3 p-10 rounded-[2.5rem] bg-linear-to-r from-emerald-500/5 to-transparent border border-emerald-500/20 backdrop-blur-md relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
              <div>
                ...
                <p className="text-slate-600 dark:text-slate-400 max-w-3xl text-base leading-relaxed">Membangun ekosistem mitigasi bencana berbasis AI untuk meningkatkan efisiensi respons darurat di Indonesia.</p>
              </div>
